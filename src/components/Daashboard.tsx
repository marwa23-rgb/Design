...
const FeatureModal = ({ feature, onClose, onGenerate, errorMessage, status, generatedImageUrl, uploadedImage, setUploadedImage }) => {
    ...
    const isGenerateDisabled = (requiresImageUpload && !uploadedImage) || (requiresTextPrompt && !promptText) || status === 'generating';
...
const DesignCard = ({ design }) => {
    return (
        <div className="bg-white rounded-2xl shadow-sm p-6 flex flex-col">
            <h4 className="text-xl font-bold text-gray-900">{design.featureTitle}</h4>
            <p className="text-gray-600 text-sm mt-1">
                Generated: {design.timestamp?.toDate()?.toLocaleString() || 'Pending...'}
            </p>
        </div>
    );
};

// New component for the Dashboard Page
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {designs.length > 0 ? (
                        ...
                        <div className="col-span-full text-center text-gray-500">
                            {authReady ? "You haven't generated any designs yet." : "Loading your designs..."}
                        </div>
                    )}
                </div>
            </div>
...
// New component for the Pricing Page
...
const ProfilePage = ({ userProfile, designsCount }) => {
    ...
    const totalCredits = 10000; // Example total credits, you can change this
            ...
            <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-xl p-8 md:p-12">
                <div className="flex flex-col md:flex-row items-center md:items-start text-center md:text-left mb-8">
                    <img
                        ...
                        src={userProfile.photoURL || "https://placehold.co/150x150/e0e0e0/ffffff?text=User"}
                        alt="User Profile"
                    />
                    <div>
                        <h2 className="text-3xl font-extrabold text-gray-900 mb-2">{userProfile.displayName || "Anonymous User"}</h2>
                        <p className="text-lg text-gray-600">{userProfile.email || "No email provided"}</p>
                        <p className="text-sm text-gray-500 mt-2">User ID: {userProfile.uid}</p>
                    </div>
                </div>
                ...
                <div className="space-y-8">
                    <div className="bg-gray-50 rounded-2xl p-6 shadow-sm">
                        ...
                        <p className="text-lg font-semibold text-gray-700">Expert Plan</p>
...
export default function App() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
            if (!currentUser) {
                ...
                const userDocRef = doc(firestoreDb, `artifacts/${__app_id}/users/${currentUser.uid}/profile/info`);
    ...
    useEffect(() => {
        ...
        try {
            const designsCollectionRef = collection(db, `artifacts/${__app_id}/users/${user.uid}/designs`);
            const q = query(designsCollectionRef);
            const unsubscribe = onSnapshot(q, (querySnapshot) => {
                const fetchedDesigns = [];
                querySnapshot.forEach((doc) => {
                    fetchedDesigns.push({ id: doc.id, ...doc.data() });
                });
                setDesigns(fetchedDesigns.sort((a, b) => (b.timestamp?.toDate() || 0) - (a.timestamp?.toDate() || 0)));
            });
            return () => unsubscribe();
        } catch (error) {
    ...
    const handleGenerate = async (feature, selectedStyle, uploadedImageFile, promptText) => {
        ...
        try {
            ...
            await addDoc(collection(db, `artifacts/${__app_id}/users/${user.uid}/designs`), {
        ...
        <div className="min-h-screen bg-gray-50 font-sans text-gray-900">
                            ...
                            <div className="relative group">
                                <button onClick={() => setCurrentPage('profile')} className="flex items-center space-x-2 text-gray-600 hover:text-blue-600 transition-colors">
                                    <img
                                        ...
                                        src={userProfile.photoURL || `https://placehold.co/32x32/e0e0e0/ffffff?text=${userProfile.displayName ? userProfile.displayName.charAt(0) : 'U'}`}
                                        alt="Profile"
                                    />
                                    <span className="hidden md:inline-block font-medium">{userProfile.displayName || "User"}</span>
                                </button>
                            </div>
                        )}
                        <div className="hidden md:block text-right text-xs text-gray-500">
                            {authReady ? `User ID: ${user?.uid || 'N/A'}` : "Authenticating..."}
                        </div>
                        <button
            ...
            {currentPage === 'home' && (
                <HomePage onSelectFeature={(feature) => {
                    ...
                    setGeneratedImageUrl(''); // Reset image when a new feature is selected
                }} />
            )}
            ...
            {currentPage === 'pricing' && (
                <PricingPage onGetStarted={() => setCurrentPage('home')} />
            )}
            {currentPage === 'profile' && (
